...
class CnnHeadPoseEstimator:
    ...
    def _allocate_yaw_variables(self):
        ...
        def model(data):
            ...
            conv1 = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(X, self.hy_conv1_weights, strides=[1, 1, 1, 1], padding='SAME'),self.hy_conv1_biases))
            pool1 = tf.nn.max_pool(conv1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
            norm1 = tf.nn.lrn(pool1, 4, bias=1.0, alpha=0.001 / 9.0, beta=0.75)

            conv2 = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(norm1, self.hy_conv2_weights, strides=[1, 1, 1, 1], padding='SAME'),self.hy_conv2_biases))
            pool2 = tf.nn.max_pool(conv2, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
            norm2 = tf.nn.lrn(pool2, 4, bias=1.0, alpha=0.001 / 9.0, beta=0.75)

            conv3 = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(norm2, self.hy_conv3_weights, strides=[1, 1, 1, 1], padding='SAME'),self.hy_conv3_biases))
            ...
            dense1 = tf.nn.relu(tf.matmul(dense1, self.hy_dense1_weights) + self.hy_dense1_biases)
            out = tf.nn.relu(tf.matmul(dense1, self.hy_out_weights) + self.hy_out_biases, name="output")
            return out
    ...
    def load_yaw_variables(self, YawFilePath):
        ...
        writer = tf.summary.FileWriter('relu_tensorboard_logs', self._sess.graph)
        writer.close()


        saver = tf.train.Saver()
        saver.save(self._sess,"relu_ncs/tf_model")
